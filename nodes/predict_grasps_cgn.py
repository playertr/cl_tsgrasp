...
class Config:
    ...
    # ckpt_dir = "/home/playert/Research/contact_graspnet/checkpoints/contact_graspnet_train_and_test"
    ckpt_dir = "/home/playert/Research/contact_graspnet/checkpoints/scene_test_2048_bs3_hor_sigma_001"
    forward_passes = 1
    arg_configs = []
cfg = Config()

def make_cgn():
    global_config = config_utils.load_config(cfg.ckpt_dir, batch_size=cfg.forward_passes, arg_configs=cfg.arg_configs)
    ...
    # Load weights
    grasp_estimator.load_weights(sess, saver, cfg.ckpt_dir, mode='test')
    return sess, grasp_estimator

sess, grasp_estimator = make_cgn()
...
def identify_grasps(pts):
    ...
    try:
        ...
        confs = torch.Tensor(pred_scores)
        grasps = torch.Tensor(pred_grasps_cam)
        grasp_offset = torch.Tensor(offset_pred)
